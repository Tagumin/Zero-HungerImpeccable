...
export default function CostOptimizer() {
  ...
  // Fetch sensitivity when crop changes
  useEffect(() => {
    if (crops.length > 0) {
  ...
  const drawChart = (history) => {
    ...
    convChartInst.current = new Chart(convChartRef.current, {
      ...
      options: { 
        ...
        scales: { 
          x: { 
            title: { display: true, text: "Iteration", font: { size: 12, weight: 600 } }, 
            ticks: { autoSkip: true, maxTicksLimit: 12 } 
          }, 
          y: { 
            title: { display: true, text: "Profit ($/ha)", font: { size: 12, weight: 600 } }, 
  ...
  const drawComparisonChart = (psoHistory, gaHistory) => {
    ...
    convChartInst.current = new Chart(convChartRef.current, {
      ...
      options: { 
        ...
        maintainAspectRatio: false, 
        plugins: { 
          legend: { position: "top", labels: { boxWidth: 14, padding: 15, font: { size: 12, weight: 600 } } } 
        }, 
        scales: { 
          x: { 
            title: { display: true, text: "Iteration", font: { size: 12, weight: 600 } }, 
  ...
  const drawHybridChart = (psoHistory, gaHistory) => {
    ...
    hybChartInst.current = new Chart(hybChartRef.current, {
      ...
      options: { 
        ...
        scales: { 
          x: { 
            title: { display: true, text: "Generation / Iteration", font: { size: 11, weight: 600 } }, 
            ticks: { autoSkip: true, maxTicksLimit: 12 } 
          }, 
          y: { 
            title: { display: true, text: "Profit ($/ha)", font: { size: 11, weight: 600 } }, 
            ticks: { callback: v => "$" + v.toLocaleString() } 
          } 
        } 
      }
  ...
  const drawSensitivityChart = (data) => {
    ...
    sensChartInst.current = new Chart(sensChartRef.current, {
      ...
      options: {
        ...
        scales: {
          x: { 
            title: { display: true, text: "Allocation Ratio (0.0 → 1.0)", font: { size: 11, weight: 600 } }, 
            ticks: { maxTicksLimit: 10 } 
          },
          y: { 
            title: { display: true, text: "Est. Profit ($/ha)", font: { size: 11, weight: 600 } }, 
            ticks: { callback: v => "$" + v.toLocaleString() } 
          }
        }
      }
  ...
  const runOptimization = async () => {
    ...
    try {
      ...
      // Delay drawing chart slightly to allow DOM to render canvas if it was hidden
  ...
  return (
    <div className="co-page-container">
      ...
      <header className="co-header" style={{marginTop: "80px"}}> {/* Margin top to account for fixed navbar if any */}
        <div>
      ...
      <div className="co-container">
        ...
        <div className="grid-2">
          {/* Parameters Card */}
          <div className="card">
            <div className="card-title">⚙️ Parameters</div>
            <div className="control-row" style={{ flexDirection: "column", alignItems: "flex-start", gap: "0.5rem" }}>
              <label>Crop (from recommendation module)</label>
              <select value={selectedCrop} onChange={(e) => setSelectedCrop(e.target.value)}>
                {crops.map(c => <option key={c} value={c}>{c.charAt(0).toUpperCase() + c.slice(1)}</option>)}
              </select>
            </div>
            
            <p style={{ fontSize: "0.85rem", color: "var(--text-muted)", margin: "1.5rem 0 0.5rem", fontWeight: 600 }}>Algorithm</p>
            ...
            <button className="run-btn" onClick={runOptimization} disabled={statusType === "running"}>
              {statusType === "running" ? "⏳ Running…" : "▶ Run Optimization"}
            </button>
            <button className="run-btn compare" onClick={runComparison} disabled={statusType === "running"}>
              ⚡ Compare Algorithms
            </button>
            <button className="run-btn hybrid" onClick={runHybrid} disabled={statusType === "running"}>
              🔗 Run Hybrid PSO→GA
            </button>
          ...
          <div className="card">
            ...
            <p style={{ fontSize: "0.72rem", color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: ".1em", marginBottom: "1rem", fontWeight: 700 }}>Optimal Resource Allocation</p>
            ...
            <div style={{ marginTop: "1.5rem" }}>
              <p style={{ fontSize: "0.72rem", color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: ".1em", marginBottom: "0.75rem", fontWeight: 700 }}>Cost Breakdown</p>
        ...
        <div className="card" style={{ marginTop: "2rem" }}>
          <div className="card-title">📈 Convergence Curve</div>
          <p style={{ fontSize: "0.85rem", color: "var(--text-muted)", marginBottom: "1rem" }}>Best profit improving over iterations</p>
        ...
        {isComparing && compResult && (
          <div className="card" style={{ marginTop: "2rem" }}>
            ...
            <div>
              <div className="winner-badge">🏆 {compResult.winner === "PSO" ? "PSO Wins!" : "Genetic Algorithm Wins!"}</div>
              <p style={{ fontSize: "0.9rem", color: "var(--text-muted)", marginBottom: "1.5rem" }}>
                Comparison results for <strong>{compResult.crop}</strong>
              </p>
            </div>
            <div className="comparison-grid">
              <div className="algo-result">
                ...
                <div className="result-item"><span>Iterations</span><span style={{color: "var(--green-mid)"}}>{compResult.results.PSO.iterations}</span></div>
                <hr style={{ margin: "0.8rem 0", border: "none", borderTop: "1px solid #ddd" }} />
                <div className="result-item" style={{ marginBottom: 0 }}><span>Water %</span><span>{compResult.results.PSO.water_pct}%</span></div>
                <div className="result-item" style={{ marginBottom: 0 }}><span>Fert %</span><span>{compResult.results.PSO.fert_pct}%</span></div>
                <div className="result-item" style={{ marginBottom: 0 }}><span>Labor %</span><span>{compResult.results.PSO.labor_pct}%</span></div>
              </div>
              <div className="algo-result ga">
                ...
                <div className="result-item"><span>Iterations</span><span style={{color: "var(--amber)"}}>{compResult.results.GA.iterations}</span></div>
                <hr style={{ margin: "0.8rem 0", border: "none", borderTop: "1px solid #ddd" }} />
                <div className="result-item" style={{ marginBottom: 0 }}><span>Water %</span><span>{compResult.results.GA.water_pct}%</span></div>
                <div className="result-item" style={{ marginBottom: 0 }}><span>Fert %</span><span>{compResult.results.GA.fert_pct}%</span></div>
                <div className="result-item" style={{ marginBottom: 0 }}><span>Labor %</span><span>{compResult.results.GA.labor_pct}%</span></div>
              </div>
            </div>
          </div>
        )}
        ...
        {isHybrid && hybridResult && (
          <>
            <div className="section-divider">New Enhancements</div>
            <div className="card" id="hybrid-card">
              <div className="card-title purple">🔗 Hybrid PSO → GA — Two-Phase Optimization</div>
              ...
              <div className="pipeline">
                ...
                <div className="pipeline-box seed">
                  ...
                  <div className="pipeline-value" style={{ color: "var(--blue)", fontSize: "0.75rem", marginTop: 6 }}>
              ...
              <div className="grid-2">
                <div>
                  <p style={{ fontSize: "0.72rem", color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: ".1em", marginBottom: "1rem", fontWeight: 700 }}>Optimal Allocation (Hybrid)</p>
                  <div className="alloc-row">
                ...
                <div>
                  <p style={{ fontSize: "0.72rem", color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: ".1em", marginBottom: "0.75rem", fontWeight: 700 }}>Two-Phase Convergence</p>
        ...
        <div className="card" style={{ marginTop: "2rem" }}>
          <div className="card-title blue">📉 Sensitivity Analysis — How Ratios Affect Profit</div>
          <p style={{ fontSize: "0.85rem", color: "var(--text-muted)", marginBottom: "1.5rem" }}>
            Move the sliders to manually set resource allocation ratios and instantly see how profit changes.
            Each curve shows profit as that resource varies while the others stay fixed at your slider values.
          </p>
          ...
          <div className="sens-sliders-grid">
            <div className="sens-slider-card water">
              ...
              <div className="sens-sub">Current usage: <span>{Math.round(sensWater * 100)}%</span> of crop's water need</div>
            </div>
            ...
            <div className="sens-slider-card fert">
              ...
              <div className="sens-sub">Current usage: <span>{Math.round(sensFert * 100)}%</span> of crop's fert need</div>
            </div>
            ...
            <div className="sens-slider-card labor">
              ...
              <div className="sens-sub">Current usage: <span>{Math.round(sensLabor * 100)}%</span> of crop's labor need</div>
            </div>
          </div>
          ...
          <div className="live-profit-box">
            ...
            <div className="live-cost-grid">
              ...
              <div className="live-cost-item">
                <div className="live-cost-label">🌱 Fert Cost</div>
          ...
          <p style={{ fontSize: "0.75rem", color: "var(--text-muted)", textAlign: "center", marginTop: "0.5rem" }}>
            Steeper curve = more sensitive to that resource. Dashed line = your current ratio.
          </p>
        </div>
